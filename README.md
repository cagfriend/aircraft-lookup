...
- **航路地图（卡片内自动显示）**：查询结果里若"当前执飞航线"有呼号，会**自动在卡片内嵌显示出地图与航路**（无需点击）；有航路则画航路点连线，**绿线为真实飞行轨迹**（OpenSky，或线上降级用 FlightAware），无航路时按大圆示意
- **完整航路卡片**：点击航段或"查看航路地图"打开弹层，含航路点明细、距离/高度/速度等指标与历史航班
- **瓦片源自动降级**：高德 → OSM 官方 → OpenStreetMap DE → OSM France (HOT)（原 CARTO 现已要求 API key 且会出水印，已弃用）
- **连线不重叠**：已有真实飞行轨迹时不再叠加航路示意连线，只标出航路点位置（避免两条几乎重合的线互相干扰）
