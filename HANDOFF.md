...
- **绘制规则（2026-09 定稿）**：**航路点 + 连线始终绘制**——有航路则蓝色折线连接航路点，无航路则灰色大圆虚线兜底，**有真实轨迹时另外叠加绿色轨迹线**（同一张图上可能同时出现蓝线与绿线）。
  - 历史：中间曾短暂改为"已有真实轨迹时不画近似连线"（理由：两线几乎重合、观感干扰），**随后按用户要求回退**，恢复为现在的画法。**不要再擅自改回抑制连线。**
  - 本地验证（假 DOM/Leaflet + 合成数据）：带轨迹 → **2 条**（绿 + 蓝）；无轨迹有航路 → 1 条（蓝）；无航路 → 1 条（灰虚线）。
