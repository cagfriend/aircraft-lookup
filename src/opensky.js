...
// 读取环境变量：Node 用 process.env；Worker 由入口传入 runtimeEnv（保留 globalThis.env 兼容兜底）
function env(name, runtimeEnv = null) {
  try {
    if (runtimeEnv && runtimeEnv[name] != null) return String(runtimeEnv[name] || '');
...
export function openskyEnabled(runtimeEnv = null) {
  return !!(env('OPENSKY_CLIENT_ID', runtimeEnv) && env('OPENSKY_CLIENT_SECRET', runtimeEnv));
}

// 请求超时：OpenSky 有时不可达，超时设短一点避免拖慢主流程
function timeoutMs(runtimeEnv = null) {
  return Number(env('OPENSKY_TIMEOUT_MS', runtimeEnv)) || 6000;
}
...
// 非 Cloudflare 中转的短熔断。中转失效时不应连带屏蔽直连尝试。
let proxyFailUntil = 0;
function proxyConfig(runtimeEnv = null) {
  return {
    base: env('OPENSKY_PROXY_BASE', runtimeEnv).replace(/\/+$/, ''),
    token: env('OPENSKY_PROXY_TOKEN', runtimeEnv),
  };
}
...
async function getToken(runtimeEnv = null) {
  if (!openskyEnabled(runtimeEnv)) return '';
  const now = Date.now();
  if (tokenCache.token && now < tokenCache.exp) return tokenCache.token;
  if (now < tokenFailUntil) return '';

  const body = 'grant_type=client_credentials'
    + '&client_id=' + encodeURIComponent(env('OPENSKY_CLIENT_ID', runtimeEnv))
    + '&client_secret=' + encodeURIComponent(env('OPENSKY_CLIENT_SECRET', runtimeEnv));
...
/**
 * 调用非 Cloudflare 的 OpenSky 中转。中转可用时返回其规范化结果；
 * 网络/认证/5xx 错误返回 null，调用方再尝试原有直连和降级路径。
 */
async function queryProxy(path, params, runtimeEnv = null) {
  if (env('OPENSKY_PROXY_DISABLE', runtimeEnv) === '1') return null;
  const config = proxyConfig(runtimeEnv);
  if (!config.base || Date.now() < proxyFailUntil) return null;
  const url = new URL(config.base + '/v1/' + path);
  Object.entries(params).forEach(([key, value]) => url.searchParams.set(key, String(value)));
  const headers = config.token ? { 'X-Aircraft-Lookup-Proxy-Token': config.token } : {};
  let res;
  try {
    res = await fetchURL(url.toString(), { timeout: timeoutMs(runtimeEnv), headers });
  } catch {
    proxyFailUntil = Date.now() + 60 * 1000;
    return null;
  }
 ...
 */
export async function queryOpenSky(icao24, runtimeEnv = null) {
  const hex = (icao24 || '').toLowerCase();
  if (!hex) return { ok: false, source: 'OpenSky', error: '缺少 ICAO24' };
  const proxied = await queryProxy('states', { icao24: hex }, runtimeEnv);
  if (proxied) return proxied;
  if (osBlocked()) return { ok: false, source: 'OpenSky', error: 'OpenSky 暂时不可达（熔断中）' };
  const token = await getToken(runtimeEnv).catch(() => '');
  const url = STATE_URL + '?icao24=' + encodeURIComponent(hex);
  let res;
  try {
    res = await fetchURL(url, { timeout: timeoutMs(runtimeEnv), headers: authHeaders(token) });
  } catch (e) {
    osMarkFail();
 ...
 */
export async function queryOpenSkyTrack(icao24, opts = {}, runtimeEnv = null) {
  const hex = (icao24 || '').toLowerCase();
  if (!hex) return { ok: false, source: 'OpenSky', error: '缺少 ICAO24' };
  const proxied = await queryProxy('tracks', { icao24: hex, maxPoints: opts.maxPoints || 240 }, runtimeEnv);
  if (proxied) return proxied;
  if (osBlocked()) return { ok: false, source: 'OpenSky', error: 'OpenSky 暂时不可达（熔断中）' };
 ...
 */
export async function queryOpenSkyFlights(icao24, opts = {}, runtimeEnv = null) {
  const hex = (icao24 || '').toLowerCase();
  if (!hex) return { ok: false, source: 'OpenSky', error: '缺少 ICAO24' };
  const hours = Math.min(Math.max(Number(opts.hours) || 24, 1), 48);
  const proxied = await queryProxy('flights', { icao24: hex, hours }, runtimeEnv);
  if (proxied) return proxied;
  const token = await getToken(runtimeEnv).catch(() => '');
  if (!token) {
